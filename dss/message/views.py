...
def create_message(data):
    ...
    if message_type=='notice':
        ...
        out_data=send_notice(TO,data['relatedDocumentLink'][0],committee_name,scheduledDate)
        print(1)
...
def forwardMessage(request):
    ...
    if message_type=="doable":
        ...
        #print("1")
    if out_data==False:
        return HttpResponse("Something went wrong in creating message. Please try after sometime")
    if 'error' in out_data:
        print('error in creating message: '+out_data['error']['message'])
        return HttpResponse(out_data['error']['message'])
    else:
        #print(out_data)
        waMessageId=out_data['messages'][0]['id']
        #print(out_data)
        ...
        print(data['receiverId'])
        print(data['messageType'])
        print('here', data['relatedDocumentLink'])
        print("2022-10-13 11:00:00")
        #print(data['doableId'])
        print(2)
        save_data={}
        ...
        try:
            ...
            print(str(execption))
            return JsonResponse("Integrity error in message", safe= False)
            print(5)
        except Exception as exception:
            print(str(exception))
            return JsonResponse("Failed to create message: Try Again",safe= False)
            print(6)
        return JsonResponse("New Message Created",safe= False)
