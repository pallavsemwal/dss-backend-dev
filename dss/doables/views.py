...
def create_doable(request):
    ...
    # del d["memberName"]
    # did=uuid.uuid1()
    relatedDocumentLink=d['relatedDocumentLink']
    d.pop('relatedDocumentLink')
...
def allCompliancePage(request):
    ...
    out={}
    sql_assigned_by="""
        select * from doables_doable d  left join auth_user u on d."assignedTo"=u."id" where "assignedBy"=%s order by d."completed";
    """
    ...
    out['assignedByPages']= paginator.num_pages
    #print(row)
    ...
    sql_assigned_to="""
        select * from doables_doable d left join auth_user u on d."assignedBy"=u."id" where "assignedTo"=%s order by d."completed"  ;
    """
    with connection.cursor() as cursor:
    ...
    out['assignedToPages']= paginator.num_pages
    #print(row)
...
def updateDoable(request):
    ...
    try:
        ...
        print(str(execption))
        return JsonResponse({'message':"Integrity error", 'success':False},safe=False, status=status.HTTP_304_NOT_MODIFIED )
    except Exception as exception:
        print(str(exception))
        return JsonResponse({'message':"Failed To Update", 'success':False},safe=False, status=status.HTTP_304_NOT_MODIFIED )
    return JsonResponse({'message':"Updated", 'success':True},safe=False, status=status.HTTP_202_ACCEPTED)
